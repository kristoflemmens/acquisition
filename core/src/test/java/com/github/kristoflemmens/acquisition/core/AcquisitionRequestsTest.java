...
public class AcquisitionRequestsTest extends UnitTestCase {
    ...
    public void get() throws Exception {
        when(eventStore.eventsFor(TestConstants.ACQUISITION_REQUEST_NUMBER)).thenReturn(history);
        when(acquisitionRequestFactory.loadFromHistory(history)).thenReturn(acquisitionRequest);

        assertThat(acquisitionRequests.get(TestConstants.ACQUISITION_REQUEST_NUMBER)).isEqualTo(acquisitionRequest);
    }
}
